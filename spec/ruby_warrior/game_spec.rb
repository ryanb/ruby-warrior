...
describe RubyWarrior::Game do
  before(:each) { @game = RubyWarrior::Game.new }
  ...
  it "should load profiles for each profile path" do
    ...
    allow(@game).to receive(:profile_paths).and_return(%w[foo/.profile bar/.profile])
  ...
  it "should ask a player to choose a profile if multiple profiles are available, but only once" do
    expect(RubyWarrior::UI).to receive(:choose).with(
      "profile",
      [:profile1, [:new, "New Profile"]],
    ).and_return(:profile1)
  ...
  it "should ask user to choose a tower when creating a new profile" do
    ...
    allow(@game).to receive(:towers).and_return(%i[tower1 tower2])
    expect(RubyWarrior::UI).to receive(:choose).with("tower", %i[tower1 tower2]).and_return(
      double(path: "/foo/bar"),
    )
    @game.new_profile
  end
  ...
  it "load towers for each tower path" do
    ...
    allow(@game).to receive(:tower_paths).and_return(%w[towers/foo towers/bar])
